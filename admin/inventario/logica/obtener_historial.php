...
try {
    ...
    if ($driver === 'sqlite') {
        $conexion->exec("CREATE TABLE IF NOT EXISTS historial_producto (
            ...
            cajas_agregar INTEGER NOT NULL,
            unidades_por_caja INTEGER NOT NULL,
            unidades_sueltas_agregar INTEGER NOT NULL,
            unidades_agregadas_total INTEGER NOT NULL,
            precio_venta_usd REAL NOT NULL,
            precio_venta_bs REAL NOT NULL,
            tasa_dolar REAL NOT NULL,
            observacion TEXT NULL
        )");
        $conexion->exec("CREATE INDEX IF NOT EXISTS idx_historial_producto_id_producto ON historial_producto(id_producto)");
    } else {
        $conexion->exec("CREATE TABLE IF NOT EXISTS historial_producto (
