...
// Cargar tasa de cambio desde Configuración (Admin)
async function cargarTasaDesdeConfiguracionAdmin() {
    try {
        ...
        if (data && data.success && data.configuraciones && data.configuraciones.tasa_dolar) {
            ...
            if (Number.isFinite(tasa) && tasa > 0) {
                ...
                // Refrescar UI dependiente de la tasa
                try { actualizarEquivalenteAbono(); } catch (_) {}
                try { mostrarCuentas(); actualizarResumen(); } catch (_) {}
            }
        }
    } catch (err) {
        console.warn('No se pudo cargar tasa desde configuración (admin):', err);
    }
}
...
function verHistorialCliente(idCliente) {
    fetch(`../logica/obtener_historial_cliente.php?id_cliente=${idCliente}`)
        ...
        .then(historial => {
            ...
            window.ultimoHistorialAdmin = historial;
...
function mostrarModalHistorial(historial) {
    ...
    const btnToggleModalDetalle = document.getElementById('btnToggleMonedaAdminDetalle');
...
function mostrarModalDetalle(data) {
    ...
    window.ultimoDetalleAdminData = data;
    // Sincronizar texto del botón de moneda del encabezado del modal
    const btnToggleModalDetalle2 = document.getElementById('btnToggleMonedaAdminDetalle');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Toggle de moneda en cabecera (Admin)
    const btnToggle = document.getElementById('btnToggleMonedaAdminCuentas');
    if (btnToggle) {
        const syncBtnText = () => {
            const m = getMonedaActual();
    ...
    // Toggle de moneda en modal Detalle/Historial (Admin)
    const btnToggleModalDetalle = document.getElementById('btnToggleMonedaAdminDetalle');
    if (btnToggleModalDetalle) {
        ...
        btnToggleModalDetalle.addEventListener('click', () => {
            ...
            localStorage.setItem('monedaActual', nuevo);
            syncBtnTextDetalle();
            if (window.modalDetalleContext === 'detalle' && window.ultimoDetalleAdminData) {
                mostrarModalDetalle(window.ultimoDetalleAdminData);
            } else if (window.modalDetalleContext === 'historial' && window.ultimoHistorialAdmin) {
                mostrarModalHistorial(window.ultimoHistorialAdmin);
            } else if (window.modalDetalleContext === 'combinado' && window.ultimoDetalleAdminData && window.ultimoHistorialAdmin) {
                mostrarModalDetalleCombinado(window.ultimoDetalleAdminData, window.ultimoHistorialAdmin);
            }
        });
    }

    // Toggle de moneda en modal Abono (Admin)
    const btnToggleModalAbono = document.getElementById('btnToggleMonedaAdminAbono');
    ...
    // Validación de montos en modal Abono (Admin)
    const toDecimal = (str) => {
...
// Ver detalle agregado por cliente
...
// Abrir modal de abono con selector de factura/fecha
function abrirModalAbonoCliente(idCliente, nombreCliente) {
    fetch(`../logica/obtener_creditos_cliente.php?id_cliente=${idCliente}`)
        ...
        .then(creditos => {
            ...
            // Guardar id del cliente y el saldo total para permitir abono general
            ...
            // limpiar campos
            const saldoInput2 = document.getElementById('saldoAbono');
            // ya seteado al total
            const montoUsdInput = document.getElementById('montoAbonoUsd');
            const montoBsInput = document.getElementById('montoAbonoBs');
            if (montoUsdInput) {
            ...
            if (montoBsInput) {
                montoBsInput.value = '';
                // En Bs, máximo equivalente al saldo total en Bs
                const tasa = getTasaCambio();
...
// Procesar abono con montos en USD y Bs
